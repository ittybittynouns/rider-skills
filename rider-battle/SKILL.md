...
3. Reply with a short list: for each, the matchId, wager (`{wager} $RIDER`), creator handle
   (or shortened wallet), and how long until it expires (`expires_at`). If empty, say there are
   no open challenges right now. Optionally offer to accept one.
...
2. `INSERT` a row into `matches` (POST, `Prefer: return=representation`) with:
   ...
   `wager` = `W`, `coin` = `'RIDER'` (or a coin the user names),
