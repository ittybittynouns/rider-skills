...
5. **Tagging (important — only tag real X handles):** the `handle` column is NOT always an
   X username. Decide per row using the `avatar` field:
   - `avatar` contains `unavatar.io/twitter/` or `twimg.com` → the player logged in with X,
     so `handle` is their X username → tag them as `@{handle}` (strip any leading `@` first;
     never output `@@`).
   - `avatar` is a Farcaster/other pfp (e.g. `imagedelivery.net`, `warpcast`, `farcaster`) →
     `handle` is a Farcaster/display name, NOT an X account → show it as plain text
     `{handle}` (no `@`), optionally noting it's a Farcaster handle. Tagging it on X would
     ping the wrong or a non-existent account.
   - `avatar` is null / `handle` looks like a shortened wallet (`0x…`) or is missing → show
     `anon` or the short wallet, no tag.
   Handles are self-entered at login, so an occasional mis-tag is possible; when unsure,
   prefer plain text over an `@`. Only ever tag the players you actually list (respect the
   requested count) — never mass-tag.
